...
if((!isset($_GET['action'])) OR ($_GET['action'] == "list")) {
	...
	{ 
		...
		{
  			$leader = '<a href="'.IP_ROOT_PATH.'adr_character.php?" . POST_USERS_URL ."='.$row['leader'].'" target="_blank">'.$Lrow['character_name'].'</a>';
		}
		...
		if($row['members'] == '') {
			...
			for($x=0;$x<$count;$x++) {
				...
				while ( $Urow = $db->sql_fetchrow($Uresult) ) { 
					if($members == '') {
						$members .= '<a href="'.IP_ROOT_PATH.'adr_character.php?" . POST_USERS_URL ."='.$newmembers[$x].'" target="_blank">'.$Urow['character_name'].'</a>';
					} else {
						$members .= ', <a href="'.IP_ROOT_PATH.'adr_character.php?" . POST_USERS_URL ."='.$newmembers[$x].'" target="_blank">'.$Urow['character_name'].'</a>';
					}
				}
	...
	{ 
		...
		while ( $Lrow = $db->sql_fetchrow($Lresult) ) { $leader = '<a href="'.IP_ROOT_PATH.'adr_character.php?" . POST_USERS_URL ."='.$row2['leader'].'" target="_blank">'.$Lrow['character_name'].'</a>'; }
...
if($_GET['action'] ==  "clanpage") {
			...
			if($row2['approvelist'] == '') {
				...
				for($x=0; $x<$count; $x++)
				{
					...
					while ( $Urow = $db->sql_fetchrow($Uresult) )
					{
						if($approvelist == '')
						{
							$approvelist .= '<a href="'.IP_ROOT_PATH.'adr_character.php?" . POST_USERS_URL ."='.$newapprovelist[$x].'" target="_blank">'.$Urow['character_name'].'</a>';
						}
						else
						{
							$approvelist .= ', <a href="'.IP_ROOT_PATH.'adr_character.php?" . POST_USERS_URL ."='.$newapprovelist[$x].'" target="_blank">'.$Urow['character_name'].'</a>';
	...
	while ( $Nrow = $db->sql_fetchrow($Nresult) ) { 
		...
		while ( $NUrow = $db->sql_fetchrow($NUresult) ) { $poster = '<a href="'.IP_ROOT_PATH.'adr_character.php?" . POST_USERS_URL ."='.$Nrow['poster'].'" target="_blank">'.$NUrow['character_name'].'</a>'; }
...
if(($_GET['action'] ==  "shoutbox") OR ($shoutbox)) {
	...
	while($Nrow = $db->sql_fetchrow($Nresult))
	{
		...
		{
			$poster = '<a href="'.IP_ROOT_PATH.'adr_character.php?" . POST_USERS_URL ."='.$Nrow['poster'].'" target="_blank">'.$NUrow['character_name'].'</a>';
		}
...
if($_GET['action'] ==  "create") {
	...
	$template->assign_vars(array( 
		...
		'FILE' => $file,
		'USERNAME' => '<a href="'.IP_ROOT_PATH.'adr_character.php?" . POST_USERS_URL ."='.$user_id.'" target="_blank">'.$userdata['username'].'</a>'
	));	
...
//		$template->set_filenames(array('body' => 'adr_clans_clp_intro_body.tpl'));
	...
	if($_GET['t'] == "news") {
	   ...
	   if(($_GET['a'] == "") OR ($news)) {
		...
		$vars = 'none';
		if(($_POST['order'] == "") && ($_POST['asc'] == "")) {
			$vars = 'get';
			// Check sort settings!
			if($_GETS['order'] == "") { $order = 'date'; } else { $order = $_GETS['order']; }
			if($_GETS['asc'] == "") { $asc = 'desc'; } else { $asc = $_GETS['asc']; }
	
			// Do SELECTED checks
			if($_GETS['order'] == 'date') { $d_selected = 'SELECTED'; } else { $d_selected = ''; }
			if($_GETS['order'] == 'poster') { $a_selected = 'SELECTED'; } else { $a_selected = ''; }
			if($_GETS['order'] == 'title') { $t_selected = 'SELECTED'; } else { $t_selected = ''; }
			if($_GETS['desc'] == 'desc') { $desc_selected = 'SELECTED'; } else { $desc_selected = ''; }
			if($_GETS['asc'] == 'asc') { $asc_selected = 'SELECTED'; } else { $asc_selected = ''; }
		}
		if(($_GETS['order'] == "") && ($_GETS['asc'] == "")) {
			$vars = 'post';
			// Check sort settings!
			if($_POST['order'] == "") { $order = 'date'; } else { $order = $_POST['order']; }
			if($_POST['asc'] == "") { $asc = 'desc'; } else { $asc = $_POST['asc']; }
	
			// Do SELECTED checks
			if($_POST['order'] == 'date') { $d_selected = 'SELECTED'; } else { $d_selected = ''; }
			if($_POST['order'] == 'poster') { $a_selected = 'SELECTED'; } else { $a_selected = ''; }
			if($_POST['order'] == 'title') { $t_selected = 'SELECTED'; } else { $t_selected = ''; }
			if($_POST['desc'] == 'desc') { $desc_selected = 'SELECTED'; } else { $desc_selected = ''; }
			if($_POST['asc'] == 'asc') { $asc_selected = 'SELECTED'; } else { $asc_selected = ''; }
		}
		if($vars == '') {
			$order = 'date';
			$asc = 'desc';
	...
	if($_GET['t'] == "members") {
		...
		if(($_GET['a'] == "") || ($members)) {
			...
			while ( $rowN = $db->sql_fetchrow($resultN) )  { 
				...
				if ( !($Lresult = $db->sql_query($Lsql)) ) { message_die(GENERAL_ERROR, 'Error retrieving data', '', __LINE__, __FILE__, $Lsql); } 
				while ( $Lrow = $db->sql_fetchrow($Lresult) ) { $leader_name = '<a href="'.IP_ROOT_PATH.'adr_character.php?" . POST_USERS_URL ."='.$rowN['leader'].'" target="_blank">'.$Lrow['character_name'].'</a>'; }
				...
				if($rowN['members'] != '') {
					...
					for($x=0;$x<$count;$x++) {
						...
						if ( !($Uresult = $db->sql_query($Usql)) ) { message_die(GENERAL_ERROR, 'Error retrieving data', '', __LINE__, __FILE__, $Usql); } 
						while ( $Urow = $db->sql_fetchrow($Uresult) ) { 
								$member_name = '<a href="'.IP_ROOT_PATH.'adr_character.php?" . POST_USERS_URL ."='.$newmembers[$x].'" target="_blank">'.$Urow['character_name'].'</a>';
						}
				...
				if($rowN['approvelist'] != '') {
					...
					for($xy=0;$xy<$count;$xy++) {
						...
						while ( $U2row = $db->sql_fetchrow($U2result) ) { 
								$approve_name = '<a href="'.IP_ROOT_PATH.'adr_character.php?" . POST_USERS_URL ."='.$newapprove[$xy].'" target="_blank">'.$U2row['character_name'].'</a>';
						}
