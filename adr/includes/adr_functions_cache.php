...
{
	global $db, $lang, $phpEx, $userdata, $table_prefix, $board_config;
...
function adr_get_poster_infos($poster_id)
{
	...
	if($cache_config[6])
	{
		...
		@include( IP_ROOT_PATH . './adr/cache/cache_posters.' . $phpEx );
...
{
	global $db, $lang, $phpEx, $userdata, $table_prefix;
	...
	$fname = IP_ROOT_PATH . './adr/cache/cache_posters'.'.' . $phpEx;
	@chmod($fname, 0666);
...
function adr_get_skill_data($target_skill)
{
	...
	global $db, $lang, $phpEx, $board_config;
	...
	{
		// All the following code has been made by Ptirhiik
		@include( IP_ROOT_PATH . './adr/cache/cache_skills.' . $phpEx );
...
function adr_update_skills()
{
	...
	$fname = IP_ROOT_PATH . './adr/cache/cache_skills'.'.' . $phpEx;
	@chmod($fname, 0666);
	$handle = @fopen($fname, 'w');
	@fwrite($handle, $res);
	@fclose($handle);
...
function adr_get_item_quality($item, $type)
{
	...
	{
		// All the following code has been made by Ptirhiik
		@include(IP_ROOT_PATH . './adr/cache/cache_item_quality.' . $phpEx);
...
//	$sql = "SELECT * FROM  " . ADR_SHOPS_ITEMS_QUALITY_TABLE;
	...
	$fname = IP_ROOT_PATH . './adr/cache/cache_item_quality'.'.' . $phpEx;
	@chmod($fname, 0666);
	$handle = @fopen($fname, 'w');
	@fwrite($handle, $res);
	@fclose($handle);
...
{
	global $db, $lang, $phpEx, $userdata, $board_config;
	...
	{
		// All the following code has been made by Ptirhiik
		@include( IP_ROOT_PATH . './adr/cache/cache_item_type.' . $phpEx );
...
//	$sql = "SELECT * FROM  " . ADR_SHOPS_ITEMS_TYPE_TABLE;
	...
	$fname = IP_ROOT_PATH . './adr/cache/cache_item_type'.'.' . $phpEx;
	@chmod($fname, 0666);
	$handle = @fopen($fname, 'w');
	@fwrite($handle, $res);
	@fclose($handle);
...
function adr_get_element_infos($element_id)
{
	...
	if($cache_config[3]){
		// All the following code has been made by Ptirhiik
		@include( IP_ROOT_PATH . './adr/cache/cache_elements.' . $phpEx );
...
function adr_update_element_infos()
{
	...
	$fname = IP_ROOT_PATH . './adr/cache/cache_elements'.'.' . $phpEx;
	@chmod($fname, 0666);
	$handle = @fopen($fname, 'w');
	@fwrite($handle, $res);
	@fclose($handle);
...
function adr_get_alignment_infos($alignment_id)
{
	...
	if($cache_config[0]){
		// All the following code has been made by Ptirhiik
		@include( IP_ROOT_PATH . './adr/cache/cache_alignments.' . $phpEx );
...
function adr_update_alignment_infos()
{
	...
	$fname = IP_ROOT_PATH . './adr/cache/cache_alignments'.'.' . $phpEx;
	@chmod($fname, 0666);
	$handle = @fopen($fname, 'w');
	@fwrite($handle, $res);
	@fclose($handle);
...
function adr_get_class_infos($class_id)
{
	...
	if($cache_config[1]){
		// All the following code has been made by Ptirhiik
		@include( IP_ROOT_PATH . './adr/cache/cache_classes.' . $phpEx );
...
function adr_update_class_infos()
{
	...
	$fname = IP_ROOT_PATH . './adr/cache/cache_classes'.'.' . $phpEx;
	@chmod($fname, 0666);
	$handle = @fopen($fname, 'w');
	@fwrite($handle, $res);
	@fclose($handle);
...
function adr_get_race_infos($race_id)
{
	...
	if($cache_config[7]){
		// All the following code has been made by Ptirhiik
		@include( IP_ROOT_PATH . './adr/cache/cache_races.' . $phpEx );
...
function adr_update_race_infos()
{
	...
	$fname = IP_ROOT_PATH . './adr/cache/cache_races'.'.' . $phpEx;
	@chmod($fname, 0666);
	$handle = @fopen($fname, 'w');
	@fwrite($handle, $res);
	@fclose($handle);
...
function adr_get_monster_infos($monster_id)
{
	...
	if($cache_config[9]){
		// All the following code has been made by Ptirhiik
		@include(IP_ROOT_PATH . './adr/cache/cache_monsters.' . $phpEx);
...
//	$sql = "SELECT * FROM " . ADR_BATTLE_MONSTERS_TABLE;
	...
	$fname = IP_ROOT_PATH . './adr/cache/cache_monsters'.'.' . $phpEx;
	@chmod($fname, 0666);
	$handle = @fopen($fname, 'w');
	@fwrite($handle, $res);
	@fclose($handle);
...
function adr_get_general_config()
{
	...
	if($cache_config[6]){
		// All the following code has been made by Ptirhiik
		@include( IP_ROOT_PATH . './adr/cache/cache_config.' . $phpEx);
...
function adr_update_general_config()
{
	...
	$fname = IP_ROOT_PATH . './adr/cache/cache_config'.'.' . $phpEx;
	@chmod($fname, 0666);
	$handle = @fopen($fname, 'w');
	@fwrite($handle, $res);
	@fclose($handle);
...
function adr_get_item_type_categories($category = 'none'){
	global $categories_text, $categories, $categories_cat, $db, $phpEx, $lang, $adr_item_type;
	adr_get_item_type(0, '');
	require_once( IP_ROOT_PATH . './adr/cache/cache_item_type.' . $phpEx );
