...
// this contains lang file, so include it after user->setup, else, setup_lang will return early
include(IP_ROOT_PATH.'rabbitoshi/includes/functions_rabbitoshi.'.$phpEx);
...
// Actions
$yourpet_name = $_POST['yourpet_name'];
$purchacing_pet = $_POST['purchacing_pet'];
$Buypet = $_POST['Buypet'];
$Vet = $_POST['Vet'];
$confirm_Vet = $_POST['confirm_Vet'];
$Feed = $_POST['Feed'];
$Shop = $_POST['Shop'];
$Drink = $_POST['Drink'];
$Clean = $_POST['Clean'];
...
$Hotel = $_POST['Hotel'];
$Hotel_out = $_POST['Hotel_out'];
$Hotel_in = $_POST['Hotel_in'];
$Evolution = $_POST['Evolution'];
$Evolution_exec = $_POST['Evolution_exec'];
$Evolution_pet = intval($_POST['evolution_pet']);
$resurrect_ok = $_POST['resurrect_ok'];
$resurrect_no = $_POST['resurrect_no'];
$Sellpet = $_POST['Sellpet'];
$confirm_sell = $_POST['confirm_sell'];
$prefs = $_POST['prefs'];
$prefs_exec = $_POST['prefs_exec'];

$start = ( isset($_GET['start']) ) ? intval($_GET['start']) : 0;

if ( isset($_GET['from']) )
{
	$Owner_list = ($_POST['from'] == 'list') ? TRUE : FALSE;
}
else if ( isset($_GET['from']) )
{
	$Owner_list = ($_GET['from'] == 'list') ? TRUE : FALSE;
}
...
if ( $board_config['rabbitoshi_enable'] && (!(is_numeric($row['owner_creature_id'])) && $searchid == $user_id ))
{
	...
	for($i = 0; $i < count($rabbit); $i++)
	{
		...
		if(file_exists(IP_ROOT_PATH."rabbitoshi/images/pets/".$rabbit[$i]['creature_img'].""))
		{ $pic = '<img src="'.IP_ROOT_PATH.'rabbitoshi/images/pets/'.$rabbit[$i]['creature_img'].'">'; }
		else { $pic = ''; }
	...
	if ($Buypet)
	{
		$yourpet_name = ( !empty($_POST['yourpet_name']) ) ? htmlspecialchars($_POST['yourpet_name']) : '';
		$purchacing_pet = ( !empty($_POST['purchacing_pet']) ) ? intval($_POST['purchacing_pet']) : '';
...
else
{
	...
	if ($Hotel || $Hotel_out || $Hotel_in )
	{
		...
		$hotel_time_days = $_POST['Hotel_time'];
	...
	if ($Evolution || $Evolution_exec )
	{
		...
		for($i = 0; $i < count($evolution_pets); $i++)
		{
			...
			if(file_exists(IP_ROOT_PATH."rabbitoshi/images/pets/".$evolution_pets[$i]['creature_img'].""))
			{ $pics = '<img src="'.IP_ROOT_PATH.'rabbitoshi/images/pets/'.$evolution_pets[$i]['creature_img'].'">'; }
			else { $pics = ''; }
	...
	{
		$pet_value = intval($_POST['pet_value']);
	...
	if ($prefs || $prefs_exec )
	{
		...
		$feed_full = intval($_POST['feed_full']);
		$drink_full = intval($_POST['drink_full']);
		$clean_full= intval($_POST['clean_full']);
	...
	if ( $board_config['rabbitoshi_enable'] )
	{
		...
		if(file_exists(IP_ROOT_PATH."rabbitoshi/images/pets/".$rabbit_config['creature_img'].""))
		{ $pic = '<img src="'.IP_ROOT_PATH.'rabbitoshi/images/pets/'.$rabbit_config['creature_img'].'">'; }
		else { $pic = ''; }
