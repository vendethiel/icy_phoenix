...
if( isset($_POST['add']) || isset($_GET['add']) )
{
	...
	$template->assign_vars(array(
		...
		"NPC_IMG2" => IP_ROOT_PATH . 'adr/images/zones/npc/' . $xxx_images[0],
		"NPC_DEF" => $xxx_images[0],
		"S_NPC_BASEDIR" => IP_ROOT_PATH . 'adr/images/zones/npc',
...
else if ( $mode != "" )
{
	switch( $mode )
	{
		case 'delete':
			...
			$npc_id = ( !empty($_POST['id']) ) ? intval($_POST['id']) : intval($_GET['id']);
		...
		case 'edit':
			...
			$template->assign_vars(array(
				...
				"S_FILENAME_OPTIONS" => $filename_list,
				"NPC_IMG" => $npc['npc_img'],
				"NPC_DEF" => IP_ROOT_PATH . 'adr/images/zones/npc/' .  $npc['npc_img'],
				"S_NPC_BASEDIR" => IP_ROOT_PATH . 'adr/images/zones/npc',
				"S_NPC_ACTION" => append_sid("admin_adr_npc.$phpEx"))
			);
		...
		case "save":
			...
			$npc_id = ( !empty($_POST['npc_id']) ) ? intval($_POST['npc_id']) : intval($_GET['npc_id']);
			$zone = $_POST['npc_zone'] ? $_POST['npc_zone'] : array('0');
			if ( in_array( '0' , $zone ) )
			    $npc_zone = '0';
			else
				$npc_zone = implode(',' , $_POST['npc_zone']);
				
			$npc_enable = intval($_POST['npc_enable']);
			$npc_price = ( $_POST['npc_price']=='' ) ? '0' : intval($_POST['npc_price']);
			$npc_name = ( isset($_POST['npc_name']) ) ? trim($_POST['npc_name']) : trim($_GET['npc_name']);
			$npc_img = ( isset($_POST['npc_img']) ) ? trim($_POST['npc_img']) : trim($_GET['npc_img']);
			$npc_msg = ( isset($_POST['npc_message']) ) ? trim($_POST['npc_message']) : trim($_GET['npc_message']);
			$npc_item = implode(',' , $_POST['npc_item']);
			$npc_msg2 = ( isset($_POST['npc_message2']) ) ? trim($_POST['npc_message2']) : trim($_GET['npc_message2']);
			$npc_msg3 = ( isset($_POST['npc_message3']) ) ? trim($_POST['npc_message3']) : trim($_GET['npc_message3']);
			$npc_view = ( $_POST['npc_view']=='' ) ? '0' : intval($_POST['npc_view']);
			$npc_points = ( $_POST['npc_points']=='' ) ? '0' : intval($_POST['npc_points']);
			$npc_exp = ( $_POST['npc_exp']=='' ) ? '0' : intval($_POST['npc_exp']);
			$npc_sp = ( $_POST['npc_sp']=='' ) ? '0' : intval($_POST['npc_sp']);
			$npc_item2 = implode(',' , $_POST['npc_item2']);
			$npc_times = ( $_POST['npc_times']=='' ) ? '0' : intval($_POST['npc_times']);
			$npc_random = ( $_POST['npc_random']=='' ) ? '0' : intval($_POST['npc_random']);
			$npc_random_chance = ( $_POST['npc_random_chance']=='' ) ? '1' : intval($_POST['npc_random_chance']);
			$npc_user_level = ( $_POST['user_level']=='' ) ? '0' : intval($_POST['user_level']);
			$npc_quest_hide = intval($_POST['npc_quest_hide']);
			$npc_quest_clue = intval($_POST['npc_quest_clue']);
			$npc_quest_clue_price = intval($_POST['npc_quest_clue_price']);

			$monster = $npc['npc_kill_monster'];
			$npc_monster = ( isset($_POST['npc_monster']) ) ? trim($_POST['npc_monster']) : trim($_GET['npc_monster']);
			$npc_monster_amount = ( $_POST['npc_monster_amount']=='' ) ? '0' : intval($_POST['npc_monster_amount']);

			if ( in_array( '0' , $_POST['class'] ) )
			    $npc_class = '0';
			else
				$npc_class = implode(',' , $_POST['class']);

			if ( in_array( '0' , $_POST['race'] ) )
			    $npc_race = '0';
			else
				$npc_race = implode(',' , $_POST['race']);

			if ( in_array( '0' , $_POST['character_level'] ) )
			    $npc_character_level = '0';
			else
				$npc_character_level = implode(',' , $_POST['character_level']);

			if ( in_array( '0' , $_POST['element'] ) )
			    $npc_element = '0';
			else
				$npc_element = implode(',' , $_POST['element']);

			if ( in_array( '0' , $_POST['alignment'] ) )
			    $npc_alignment = '0';
			else
				$npc_alignment = implode(',' , $_POST['alignment']);

			if ( in_array( '0' , $_POST['npc_visit'] ) )
			    $npc_visit = '0';
			else
				$npc_visit = implode(',' , $_POST['npc_visit']);

			if ( in_array( '0' , $_POST['npc_quest'] ) )
			    $npc_quest = '0';
			else
				$npc_quest = implode(',' , $_POST['npc_quest']);
		...
		case "savenew":
			...
			if ( in_array( '0' , $_POST['npc_zone'] ) || $_POST['npc_zone'] == '' )
			    $npc_zone = '0';
			else
				$npc_zone = implode(',' , $_POST['npc_zone']);
			$npc_enable = intval($_POST['npc_enable']);
