...
else if ( $mode != "" )
{
	switch( $mode )
	{
		case 'delete':
			...
			$book_id = ( !empty($_POST['id']) ) ? intval($_POST['id']) : intval($_GET['id']);
		...
		case "save":
			...
			$book_id = ( !empty($_POST['book_id']) ) ? intval($_POST['book_id']) : intval($_GET['book_id']);
			$book_name = ( isset($_POST['book_name']) ) ? trim($_POST['book_name']) : trim($_GET['book_name']);
			if ( in_array( '0' , $_POST['book_zone'] ) )
			    $book_zone = '0';
			else
				$book_zone = implode(',' , $_POST['book_zone']);
			$book_details = ( isset($_POST['book_details']) ) ? trim($_POST['book_details']) : trim($_GET['book_details']);
			$book_difficulty = intval($_POST['book_difficulty']);
			$book_false = intval($_POST['book_false']);
			$book_crafting = intval($_POST['book_crafting']);
