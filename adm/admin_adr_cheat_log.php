...
$ban_board = isset($_POST['ban_board']);
$ban_adr = isset($_POST['ban_adr']);

if( isset($_POST['cheat_id']) || isset($_GET['cheat_id']) )
{
	$cheat_id  = ( isset($_POST['cheat_id']) ) ? $_POST['cheat_id'] : $_GET['cheat_id'];
...
{
	// Define some values
	$items = ( isset($_POST['action_box']) ) ?  $_POST['action_box'] : array();
	...
	switch($mode)
	{
		...
		case 'punish':
			...
			{
				$jailed_userid  = ( isset($_POST['u']) ) ? $_POST['u'] : $_GET['u'];
				$jailed_userid  = intval($jailed_userid);
			}
			if( isset($_POST['cheat']) || isset($_GET['cheat']) )
			{
				$jailed_cheat_type = ( isset($_POST['cheat']) ) ? $_POST['cheat'] : $_GET['cheat'];
...
$pagination 	= generate_pagination(IP_ROOT_PATH . "admin/admin_adr_cheat_log.$phpEx?mode=", $total, $board_config['posts_per_page'], $start). '&nbsp;';
